...
- **多阶段建模 (Multi-stage Modeling)**: 对多阶段编程 (MSP) 进行形式化，精确描述前一阶段程序如何通过特化 (Specialization) 生成后一阶段的程序。
- **架构验证 (Architecture Verification)**: 利用依赖类型 (DT) 捕捉语言实现细节，静态排除环境泄露与阶段错配，确保编译器架构的数学正确性。
