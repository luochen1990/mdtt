...
编译器载体 CompilerCarrier, 以及基于函数空间的 NbE 求值器.
...
故 `.compiler` 在 hostSem 中为死分支 (Unit), 模型层以见证令牌 + unroll 重构
的方式满足 Model 的编译器接口 (见 Stlc/Model.lean); 忠实载体建模列为未来工作.
-/
...
- `compiler S T` = Unit **死分支** (hostSem 单独使用时不可达; 模型层覆写为 CompilerCarrier).
-/
def hostSem : Ty → Type
  | .nat => Nat
  | .bool => Bool
